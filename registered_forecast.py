...
# Registration data taken from https://hedgescompany.com/automotive-market-research-statistics/auto-mailing-lists-and-marketing/
...
results.plot_diagnostics(lags=5, figsize=(15, 12))
...
mse = ((mte_forecast - mte_truth['US Cars (Millions)']) ** 2).mean()
print('The Mean Squared Error (MSE) of the forecast is {}'.format(round(mse, 2)))
print('The Root Mean Square Error (RMSE) of the forcast: {:.4f}'
      .format(np.sqrt(sum((mte_forecast-mte_truth['US Cars (Millions)'])**2)/len(mte_forecast))))
      
pred_dynamic = results.get_prediction(start=pd.to_datetime('2016'), dynamic=True, full_results=True)
pred_dynamic_ci = pred_dynamic.conf_int()

ax = mte['2010':].plot(label='observed', figsize=(20, 15))
...
# Compute the mean square error
mse = ((mte_forecast - mte_orginal['US Cars (Millions)']) ** 2).mean()
print('The Mean Squared Error (MSE) of the forecast is {}'.format(round(mse, 2)))
print('The Root Mean Square Error (RMSE) of the forcast: {:.4f}'
      .format(np.sqrt(sum((mte_forecast-mte_orginal['US Cars (Millions)'])**2)/len(mte_forecast))))
